        ...
        <div class="card">
          <div class="table-responsive">
            <table>
              ...
              <tbody>
                <tr v-for="booking in reservations" :key="booking.id">
                  <td>{{ booking.id }}</td>
                  ...
                  <td><span :class="['status-badge', `status-${booking.status.code}`]">{{ booking.status.text }}</span></td>
                </tr>
              </tbody>
            </table>
          </div>
...
.chart-card { /* chart-placeholder 스타일은 이제 필요 없습니다. */ }
...
.status-badge { padding: 4px 10px; border-radius: 12px; font-size: 0.85rem; font-weight: 600; color: white; }
.status-confirmed { background-color: #27AE60; }
.status-pending { background-color: #F39C12; }
.status-completed { background-color: #8E44AD; }
